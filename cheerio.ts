...
const getHtml = async url => {
  await agent
    ...
    .end((err, res) => {
        ...
        item.city.forEach(async (city, i) => {
          ...
          await agent
            ...
            .end((err, res) => {
              ...
              // 获取到年级内容
              provinceList[index].city[i]?.districtList.forEach(
                async (district, c) => {
                  ...
                  await agent
                    ...
                    .end(async (err, res) => {
                      ...
                      provinceList[index].city[i].districtList[
                        c
                      ].起始年级 = EnglishList[0].text?.split('英语')?.[0]
                      provinceList[index].city[i].districtList[
                        c
                      ].版本 = EnglishList[0].title?.split('义务教育')?.[0]
                      provinceList[index].city[i].districtList[c].标题 =
                        EnglishList[0].title
                      provinceList[index].city[i].districtList[c].简称 =
                        EnglishList[0].text
                      provinceList[index].city[i].districtList[
                        c
                      ].备注 = EnglishList[0].text.match(/\((.+?)\)/g)?.[0]
                    })
                }
              )
            })
      ...
      setTimeout(() => {
        // // 写入文件
        const writerStream = fs.createWriteStream(`${provinceText}.json`)
